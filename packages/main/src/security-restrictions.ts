...
app.on('web-contents-created', (_, contents) => {
  ...
  contents.session.setPermissionRequestHandler((webContents, permission, callback) => {
    ...
    if (!permissionGranted && import.meta.env.DEV) {
      logger.warn(`${origin} requested permission for '${permission}', but was rejected.`);
    }
  });
  ...
  contents.setWindowOpenHandler(({url}) => {
    ...
    if (ALLOWED_EXTERNAL_ORIGINS.has(origin as `https://${string}`)) {
      ...
      logger.warn(`Blocked the opening of a disallowed origin: ${origin}`);
    }
  ...
  contents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    if (!ALLOWED_ORIGINS_AND_PERMISSIONS.has(origin)) {
      if (import.meta.env.DEV) {
        logger.warn(`A webview tried to attach ${params.src}, but was blocked.`);
      }
