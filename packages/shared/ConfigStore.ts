...
const ConfigStoreZ = z.object({
  ...
  timeZoneOffset: z.tuple([z.string(), z.number()]).default(timeZoneDefault).catch(timeZoneDefault),
...
export function loadConfigFromJson(configJson: string, errorHandler?: ErrorMessageHandler) {
  ...
  if (!parsed.success) {
    ...
    console.error('Failed to parse config', {json: configJson, parseError: parsed.error});
    return;
  }
  console.log('Success fully loaded config data');
...
export function loadIntervalConfigFromJson(intervalConfigJson: string): boolean {
  ...
  if (!parsed.success) {
    console.error('Failed to parse interval config', {json: intervalConfigJson, parseError: parsed.error});
    return false;
  }
  for (const key of intervalKeys) {
...
export function setConfigStoreValue<KeyString extends keyof(ConfigStore)>(key: KeyString, value: ConfigStore[KeyString]) {
  if (isRenderer()) {
    console.error('Can not setConfigStoreValue in renderer, use window.bridgeApi.sendConfigStoreValue()');
    return;
  }
