...
export function loadCalibFromJson(calibJson: string, errorHandler?: ErrorMessageHandler) {
  ...
  if (!parsed.success) {
    console.error('Failed to parse calib', {json: calibJson, parseError: parsed.error});
    return;
  }
  console.log('Success fully loaded calib data');
  updateCalibStore(parsed.data);
  console.log(gCalibData);
}
...
export function setCalibStoreValue<KeyString extends keyof(CalibStore)>(key: KeyString, value: CalibStore[KeyString]) {
  if (isRenderer()) {
    console.error('Can not setCalibStoreValue in renderer, use window.bridgeApi.sendCalibStoreValue()');
    return;
  }
